module.exports = function (args) {
  ...
  function copyBuildCommon() {
    log.info('\nCopying ' + buildCommonFile + ' to ' + csProjDir + '\\..\\..\\build');

    if (fs.existsSync(csProjDir + '/../../build/' + buildCommonFile)) {
      log.warn('[SKIPPED]');
    }
    else {
      ...
      if (!fs.existsSync(csProjDir + '/../../build')) {
        fs.mkdirSync(csProjDir + '/../../build');
      }
      fs.writeFileSync(csProjDir + '/../../build/' + buildCommonFile, buildCommonContent);
      log.success('[DONE]');
    }
  }
